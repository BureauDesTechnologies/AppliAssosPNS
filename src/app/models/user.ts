...
export class User {
    ...
    static fromJSON(doc): User {
        const user = new User(doc.firstName, doc.lastName, doc.mail, doc.photoUrl, doc.roles,
            (doc.canPublishAs === null || doc.canPublishAs === undefined) ? [] : doc.canPublishAs,
            doc.userId);
        if ((doc.state !== null && doc.state !== undefined)) {
            user.state = doc.state;
        }
        if ((doc.subscriptions !== null && doc.subscriptions !== undefined)) {
